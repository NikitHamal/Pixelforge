...
`).join('\n');
    const anims = at.states.map(s => {
      /* Godot's per-frame `duration` is a MULTIPLIER of the animation's
         1/speed, not seconds. Writing 1.0 for everything threw away the hold
         frames templates author deliberately — a two-frame blink spent as
         long open as shut. */
      const unit = 1000 / s.fps;
      const list = at.frames.slice(s.from, s.to + 1).map((f, k) => `{
"duration": ${Math.max(0.05, f.duration / unit).toFixed(3)},
...
`;
  ...
  /* Unity keys every asset by a 32-hex guid. A meta file without one is
     treated as unimported and rewritten from scratch on open, which discards
     every sprite rect below. Derived from the sheet name so re-exporting the
     same sprite keeps the same guid and existing scene references survive. */
  function guidFor(name) {
    let h = 0x811c9dc5;
    let out = '';
    for (let k = 0; k < 4; k++) {
      for (let i = 0; i < name.length; i++) { h ^= name.charCodeAt(i) + k; h = Math.imul(h, 0x01000193) >>> 0; }
  ...
  function unity(at) {
    const rects = at.frames.map((f, i) => `    - serializedVersion: 2
      ...
      spriteID: ${guidFor(at.name + '#' + i)}
      internalID: ${i + 1}`).join('\n');
    return `fileFormatVersion: 2
guid: ${guidFor(at.name)}
TextureImporter:
  textureType: 8
  spriteMode: 2
...
`;
  ...
  function phaserAnims(at, opts = {}) {
    /* The caller may have written the atlas under a disambiguated name (the
       CLI folds the target id in when two targets share an extension), so the
       loader has to be told the real filename rather than assuming one. */
    const atlasFile = opts.atlasFile || `${at.name}.json`;
...
export function preload(scene) {
  scene.load.atlas('${at.name}', '${at.image}', '${atlasFile}');
}
...
`;
  ...
  function cssAnim(at) {
    ...
    /* One-shot states carry animation-fill-mode: forwards. Without it the
       element snaps back to its own background-position the tick the
       animation ends, so a death pose popped upright on its last frame. */
    const head = `/* Generated by PixelForge Studio — ${at.image} */\n\n`;
    return head + at.states.map(s => {
  ...
  background-position: -${at.frames[s.from].x}px -${at.frames[s.from].y}px;
  animation: ${cls}-${safe(s.name)} ${(n / s.fps).toFixed(3)}s steps(1, end) ${s.loop ? 'infinite' : '1'};
  animation-fill-mode: ${s.loop ? 'none' : 'forwards'};
}
