...
PF.Library = (() => {
  ...
  function registerAll() {
    ...
    /* ---- Hearthhold: an overhead settlement kit for colony and siege games ----
       Every other building pack here is an elevation. These are drawn for a
       camera looking almost straight down, so a structure occupies its tile
       footprint instead of standing in front of it. ---- */
    const HD = () => PF.Hold;
    add('hold_ground', 'Hold Terrain', 'World', '16-tile 64x64 settlement sheet: meadow, tufted and flowering grass, road, cart ruts, tilled and watered furrows, sand, deep and shallow water, cobble, planks, gravel, ash, moss and rubble.', ['settlement', 'tileset', 'overhead', 'terrain', 'colony'], () => HD().groundSuite(), { w: 64, h: 64, featured: true });
    add('hold_palisade', 'Palisade Autotile', 'World', '16-piece 64x64 connection-mask sheet of split-log walling \u2014 one cell per neighbour combination, so runs, corners, tees and crosses all turn correctly against a tile grid.', ['settlement', 'wall', 'autotile', 'palisade', 'defence'], () => HD().palisadeSuite(), { w: 64, h: 64, featured: true });
    add('hold_rampart', 'Rampart Autotile', 'World', '16-piece 64x64 connection-mask sheet of coursed stone walling with crenel notches, sharing the palisade geometry so the two upgrade into each other cleanly.', ['settlement', 'wall', 'autotile', 'stone', 'defence'], () => HD().rampartSuite(), { w: 64, h: 64 });
    add('hold_homes', 'Hold Homes', 'World', 'Six overhead dwellings on a shared roof-and-eave form: canvas tent, thatched hut, cottage, house, a teal-roofed hall with a stone portico, and a striped market stall.', ['settlement', 'building', 'house', 'overhead', 'colony'], () => HD().homeSuite(), { w: 32, h: 32, featured: true });
    add('hold_works', 'Hold Workplaces', 'World', 'Eight production buildings, three of them animated: woodcutter, open quarry pit, a smoking kiln, a forge with a live hearth, storehouse, stilted granary, a well with rippling water and a tavern.', ['settlement', 'building', 'industry', 'overhead', 'colony'], () => HD().workSuite(), { w: 32, h: 32, featured: true });
    add('hold_keep', 'Hold Defences', 'World', 'Six defence pieces: a barred gate and its open twin on one footprint, an overhead tower ring, barracks, a burning brazier and a rippling banner.', ['settlement', 'defence', 'gate', 'tower', 'siege'], () => HD().keepSuite(), { w: 32, h: 32, featured: true });
    add('hold_nodes', 'Hold Resources', 'World', 'Eight harvestable nodes on trampled ground: coal, iron, gold, stone, a terraced clay pit, a berry bush, a cut stump and a sapling.', ['settlement', 'resource', 'mining', 'node', 'gather'], () => HD().nodeSuite(), { w: 32, h: 32 });
    add('hold_site', 'Hold Construction', 'World', 'Three progressive build stages on one scaffold \u2014 bare frame, walls up, roof boarded \u2014 plus the scorched ruin a razed building leaves behind.', ['settlement', 'construction', 'build', 'scaffold', 'ruin'], () => HD().siteSuite(), { w: 32, h: 32 });
